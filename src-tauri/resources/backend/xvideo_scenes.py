...
def target_scene_count(total_duration, total_words, sec_per_scene=9.0):
    import math
...
def llm_segment_scenes(narration, n_scenes, timeout=240):
    """Use the local LLM to split narration into exactly n_scenes coherent scenes.
    ...
    try:
        ...
        data = _json.loads(out.get("response", "") or "{}")
...
def build_scenes_smart(narration, images, total_duration, min_scene=2.5, use_llm=True):
    ...
    segs = llm_segment_scenes(narration, n_scenes) if use_llm else None
    if not segs:
...
def build_scenes(narration, images, total_duration, min_scene=2.5):
    ...
    import math
    # CEO target (2026-06-06): ~8-10s per scene, i.e. a 60s video ~ 6-7 scenes.
...
def make_html_scenes(aspect, title, date_str, duration, voice_path, bgm_path,
    ...
    safe_title = _html.escape(title[:160])
    ...
    for i, sc in enumerate(scenes):
        ...
        num = _html.escape(next((w for w in txt.split() if any(ch.isdigit() for ch in w)), str(i+1)))
        hue = (i * 47) % 360
        if sc.get("image"):
...
.media-card{{position:absolute;top:{pct(0.24)}px;right:{int(w*0.06)}px;width:{int(w*0.34)}px;height:{pct(0.30)}px;border-radius:{pct(0.028)}px;background-size:cover;background-position:center;box-shadow:0 {pct(0.018)}px {pct(0.05)}px rgba(0,0,0,.45);border:1px solid rgba(255,255,255,.22);overflow:hidden}}
...
.copy-card{{position:absolute;left:{int(w*0.06)}px;right:{int(w*0.06)}px;bottom:{pct(0.13)}px;min-height:{pct(0.18)}px;padding:{pct(0.026)}px {pct(0.028)}px;border-radius:{pct(0.032)}px;background:linear-gradient(135deg,rgba(255,255,255,.16),rgba(255,255,255,.07));border:1px solid rgba(255,255,255,.23);backdrop-filter:blur(14px);font-size:{pct(0.024)}px;font-weight:850;color:#fff;line-height:1.32;text-shadow:0 2px 16px rgba(0,0,0,.75);display:flex;align-items:center;justify-content:center;text-align:left}}
...
.data-badge b{{font-size:{pct(0.04)}px;line-height:1}}.data-badge span{{font-size:{pct(0.01)}px;font-weight:900;text-transform:uppercase}}
...
.vtitle{{position:absolute;top:{pct(0.13)}px;left:{int(w*0.06)}px;right:{int(w*0.06)}px;font-size:{pct(0.032)}px;font-weight:900;color:#fff;line-height:1.14;text-shadow:0 3px 22px rgba(0,0,0,.85);z-index:20}}
