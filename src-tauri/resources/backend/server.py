    ...
    try:
        ...
        if ('xvideo_pexels' in globals() and xvideo_pexels) and len(local_imgs) < need and (req.use_stock if req.use_stock is not None else True):
            try:
                ...
                kws_used = xvideo_pexels.derive_keywords(title, content)
                stock_urls = []
                for kw in kws_used:
...
def llm_rewrite(text, title, style, length, tone):
    ...
    payload = json.dumps({
        ...
        "options": {"temperature": 0.8 if tone or style else 0.7, "num_predict": words * 3 + 200},
    }).encode("utf-8")
